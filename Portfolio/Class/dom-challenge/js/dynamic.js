...
const createActionCard = (text, priority) => {
    ...
    let badgeColor = "bg-secondary"; // Default badge color
...
const updateCounter = () => {
    ...
    const completedCards = actionList.querySelectorAll('.impact-card.completed').length;
    cardCounter.textContent = `Open tasks: ${totalCards - completedCards}`;
};

// ==========================================
// 4. TODO: FORM SUBMIT LISTENERS
// ==========================================
actionForm.addEventListener('submit', (e) => {//<form> from html, button is type subit in html
    // Prevent browser reload
    e.preventDefault(); //Prevent default form submission behavior, stop regular life cycle of the application

    // Extract input text, instantiate a card, append to target list, reset forms, update counts
    const text = actionInput.value.trim(); // Get the input value and trim the extra characters/spaces (tabs, spaces...)
    const priority = prioritySelect.value; // Get the selected priority value
    if(text === "") return; // Exit if input is empty
    const newCard = createActionCard(text, priority); // Create a new card using the extracted text and priority
    actionList.appendChild(newCard); //add the new card
    //we need to clean after ourselves:
    actionInput.value= ""; // Reset the input field to empty string
    prioritySelect.value = "medium"; // Reset the priority select to default value
    updateCounter(); // Update the card counter
});
...
saveBtn.addEventListener('click', () => {
    ...
    const actionCards = actionList.querySelectorAll('li'); //this should return all the li elements I introcued earlier 
    const exportData = []; //container that should hold all the data I want to add

    // 2. Loop through active elements and scrape current UI state into an array
    /*for(let i=0; i<actionCards.length; i++){ //or we can use (const card in actionCards)
        const card = actionCards[i];
    }*/ //a very common way we already know
    //this is the very java scripty version:
    ...
    console.log(exportData); //check if the data is being exported correctly
